...
def delete_intermediate_files():
    ...
    filelist = [f for f in os.listdir(directory) if f.endswith(
        ".json") or f.endswith(".pkl")]
    for f in filelist:
...
def new_url_run(report_url, report_company, report_year, downloaded=False):
    ...
    report_output_file_path = upload_pdf(
        report_url, report_company, report_year, downloaded)
...
#     report_output_file_path = "data/new_report/Canada Pension2017.json"
    ...
    # delete all json files, keeping the empty wordcloud, chart, table folders
    delete_intermediate_files()
